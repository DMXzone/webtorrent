...
Torrent.prototype._drain = function () {
  ...
  if (process.env.socks_proxy) {
    var parts = addrToIPPort(process.env.socks_proxy)
    var config = {
      proxy: {
        ipaddress: parts[0],
        port: parts[1],
    ...
    require('socks').createConnection(config, onSocket)
  } else {
    onSocket(null, net.connect(opts));
  }
                                    
  function onSocket(err, socket) {
    if (err) {
      return peer.destroy(err)
    }
    
    var conn = peer.conn = socket //net.connect(opts)
